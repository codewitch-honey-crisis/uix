...
It should be noted that `update()` is in essence a coroutine, and as such it can break up its work into multiple parts to avoid blocking for as long as it otherwise would. In this case, if you pass `false`, as in `update(false)` only one transfer to the LCD will occur in that iteration. You'd often need to call it multiple times (until `is_dirty()` is `false`) to do a complete refresh. This mode is useful if you're doing some other intensive task, like playing audio on the same thread and you can't have the screen blocking, at least as much as it otherwise would. Do not call `invalidate()` on anything or otherwise modify controls while updating.
