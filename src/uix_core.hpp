...
template <class T>
struct uix_remove_reference {
    typedef T type;
};
template <class T>
struct uix_remove_reference<T&> {
    typedef T type;
};
template <class T>
struct uix_remove_reference<T&&> {
...
class control_surface_impl<BitmapType, true> : public control_surface_impl_base<BitmapType>, public gfx::blt_span {
   ...
   public:
    ...
    virtual const gfx::gfx_cspan cspan(point16 location) const override {
        ...
        // clamp A: distance from location to the surface's right edge (surface space)
        result.length = gfx::math::max_(this->m_rect.x2 - this->m_rect.x1 + 1 - location.x, 0) * pixel_type::byte_alignment;
        // clamp B: distance from oloc to the bitmap's right edge (BITMAP space).
        // must use oloc.x, not location.x -- the bitmap width is measured in bitmap
        // coordinates, so mixing in the surface-space column under-reports by
        // |m_rect.x1| on offset tiles and drops the boundary column.
        result.length = gfx::math::min_(result.length,
                                        (this->m_bitmap.dimensions().width - oloc.x) * pixel_type::byte_alignment);
        return result;
    }
    virtual gfx::gfx_span span(point16 location) const override {
        ...
        result.length = gfx::math::max_(this->m_rect.x2 - this->m_rect.x1 + 1 - location.x, 0) * pixel_type::byte_alignment;
        // clamp B: distance from oloc to the bitmap's right edge (BITMAP space).
        // must use oloc.x, not location.x (see cspan note above).
        result.length = gfx::math::min_(result.length,
                                        (this->m_bitmap.dimensions().width - oloc.x) * pixel_type::byte_alignment);
        return result;
    }
